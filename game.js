    ...
    try {
        ...
        for (const mode in modeNames) {
            ...
            if (modeScores.length === 0) {
                html += `<p class="no-scores">첫 번째 기록의 주인공이 되어보세요!</p>`;
            } else {
...
showYuliMode() {
    ...
    if (yuliModeCard) {
        ...
        message.textContent = '🎉 특별 모드를 발견하셨네요! 🎉';
...
startNewGame(mode) {
    ...
    if (mode.includes('yuli')) {
        const message = document.createElement('div');
        message.innerHTML = `
            <div style="font-size: 2rem; margin-bottom: 10px;">✨ 특별 모드 ✨</div>
            <div style="font-size: 1.2rem;">엄선된 34개국 도전!</div>
        `;
        message.style.cssText = `
            position: fixed;
            top: 50%;
...
updateQuestionInfo() {
	...
	if (wrongDisplay) {
		wrongDisplay.textContent = `도전 기회: ${remainingWrong}/10`;
		if (remainingWrong <= 3) {
...
showResult(isCorrect) {
    ...
    if (isCorrect) {
        ...
        resultDiv.innerHTML = `
            <strong>아쉬워요! 다음에는 더 잘하실 거예요! 😊</strong>
...
showFinalScore() {
	...
	if (this.gameOver) {
		// 게임 종료 상태
		headerText = '😊 수고하셨습니다!';
		scoreText = `${this.score}개 맞추셨어요!<br><strong style="font-size: 1.5em; color: #ffeaa7;">${finalScore}/1000점</strong><br>다시 도전해보세요!`;
	} else if (totalAttempted < this.totalQuestions) {
		headerText = '🎯 멋진 도전이었어요!';
	...
	if (this.gameOver) {
		if (finalScore >= 800) {
			message = '정말 잘하셨어요! 다음엔 더 좋은 결과가 있을 거예요! 😊';
			emoji = '😊';
		} else if (finalScore >= 600) {
			message = '훌륭한 실력이에요! 다시 도전해보세요! 💪';
			emoji = '💪';
		} else if (finalScore >= 400) {
			message = '좋은 시작이에요! 조금만 더 연습하면 훨씬 나아질 거예요! 📚';
			emoji = '📚';
		} else {
			message = '계속 도전하세요! 연습하면 실력이 쭉쭉 늘 거예요! 🌱';
			emoji = '🌱';
		}
	} else {
		// 짜국이 모드인 경우 특별한 메시지
		if (this.currentMode.includes('yuli')) {
			if (finalScore === 1000) {
				message = '완벽해요! 모든 나라를 마스터하셨네요! ✨🏆';
				emoji = '🏆';
			} else if (finalScore >= 900) {
				message = '대단해요! 거의 완벽한 실력이에요! ✨';
				emoji = '🌟';
			} else if (finalScore >= 700) {
				message = '훌륭하세요! 정말 잘하셨어요! ✨';
				emoji = '😊';
			} else if (finalScore >= 500) {
				message = '좋은 실력이에요! 계속 연습해보아요! ✨';
				emoji = '💪';
			} else {
				message = '좋은 시작이에요! 다음엔 더 잘하실 거예요! ✨';
				emoji = '🌱';
			}
		} else {
			if (totalAttempted === this.totalQuestions && finalScore === 1000) {
				...
				message = '시작이 반이에요! 다음엔 더 좋은 결과가 있을 거예요! 🌱';
				emoji = '🌱';
			}
		}
	}

	const totalCountriesText = this.currentMode.includes('yuli') ? '엄선된 34개국' : `전 세계 ${CountryUtils.getTotalCount()}개국`;
	...
	messageDiv.innerHTML = `
		...
		<div style="margin-top: 15px; padding: 15px; background: rgba(102,126,234,0.1); border-radius: 15px; font-size: 1rem; color: #333;">
			...
			최종 점수: <strong>${finalScore}/1000점</strong> | 정답률: ${attemptedPercentage}%
		</div>
	`;
...
showRankResult(rank, playerName) {
    const modeNames = {
        ...
        'capital-easy-yuli': '특별: 국기+나라 → 수도',
        'capital-hard-yuli': '특별: 국기 → 수도',
        'capital-to-flag-yuli': '특별: 수도 → 국기'
    };
    ...
    if (rank === 1) {
        rankEmoji = '🥇';
        rankMessage = '축하합니다! 1등이에요!';
    } else if (rank === 2) {
        rankEmoji = '🥈';
        rankMessage = '대단해요! 2등이에요!';
    } else if (rank === 3) {
        rankEmoji = '🥉';
        rankMessage = '훌륭해요! 3등이에요!';
    } else if (rank <= 5) {
        rankEmoji = '🏆';
        rankMessage = `멋져요! ${rank}등이에요!`;
    } else if (rank <= 10) {
        rankEmoji = '⭐';
        rankMessage = `잘하셨어요! ${rank}등이에요!`;
    } else {
        rankEmoji = '✨';
        rankMessage = `${rank}등이에요! 다음엔 더 좋은 결과가 있을 거예요!`;
    }
    ...
    popup.innerHTML = `
        ...
        <div style="font-size: clamp(0.9rem, 2.5vw, 1.1rem); margin-bottom: 20px; opacity: 0.9;">
            [${modeName}] 모드에서<br>
            <strong style="font-size: clamp(1.1rem, 3vw, 1.3rem); color: #ffeaa7;">${rank}위</strong>로 등록되었어요!
        </div>
